    ...
    if (await FlutterContacts.requestPermission()) {
      ...
      var decodedContacts = contacts.where((contact) {
        return contact.phones.isNotEmpty &&
            _formatPhoneNumber(contact.phones[0].number) !=
                _formatPhoneNumber(userEntity!.phone);
      }).map((contact) {
        return ContactUser(
      ...
      for (var contact in decodedContacts) {
        if (contact.phone.isNotEmpty) {
          registrationChecks
              .add(checkIfContactRegistered(contact.phone).then((isRegistered) {
            contact.isRegistered =
                isRegistered; // Actualiza el estado de registro
          }));
        }
      }
    ...
    if (query.isEmpty) {
      setState(() {
        filteredContacts = List.from(
            contactsList); // Mostrar todos los contactos si no hay búsqueda.
      });
    } else {
      setState(() {
        filteredContacts = contactsList.where((contact) {
          return contact.phone
              .contains(query); // Filtrar por número de teléfono.
        }).toList();
      });
    }
  }
                        ...
                        SliverList(
                          delegate: SliverChildBuilderDelegate(
                            ...
                            (context, index) {
                              ...
                              debugPrint(
                                  "Los contactos registrado es: ${contact.name} con el número: ${contact.phone} y si esta registrado: ${contact.isRegistered}");
                              return Padding(
